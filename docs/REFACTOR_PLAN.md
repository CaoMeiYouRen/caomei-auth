...
| G2   | 控制文件长度     | 单文件长度 ≤ 600 行，新增 ESLint `max-lines` 规则守护                                                    |
...
-   **静态规则**：
    ...
    -   `eslint.config.js` 已配置 `'max-lines': [1, { max: 600 }]`。
...
-   **实施方案**：
    ...
    -   **新增规划 (2025-12-08)**：
        -   ✅ `useClipboard`：封装 `navigator.clipboard` 与 Toast 提示，统一复制交互（已实现于 `composables/utils/use-clipboard.ts`）。
        -   ✅ `useConfirmAction`：封装 `useConfirm`，预设删除、登出等常用场景配置（已实现于 `composables/utils/use-confirm-action.ts`）。
        -   ✅ `usePageMeta`：统一 SEO 标题格式与 Open Graph 标签生成（已实现于 `composables/utils/use-page-meta.ts`）。
        -   ✅ `useLogout`：统一登出逻辑与状态管理（已实现于 `composables/utils/use-logout.ts`）。
...
-   **现状分析**：
    ...
    -   `server/api/admin/sync-admin-role.post.ts`：手动检查 `userId`。 -> ✅ 已接入 Zod Schema
    -   `server/api/oauth/revoke-consent.post.ts`：手动检查 `clientId`。 -> ✅ 已接入 Zod Schema
