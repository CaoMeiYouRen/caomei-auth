...
| `redirect_uri` | string | 是 | 重定向URI，必须与注册时的URI匹配 |
| `scope` | string | 否 | 请求的权限范围，多个用空格分隔 |
| `state` | string | 推荐 | 状态参数，用于防止CSRF攻击 |
| `code_challenge` | string | 否 | PKCE挑战码（当前版本不支持） |
| `code_challenge_method` | string | 否 | PKCE挑战方法（当前版本不支持） |
...
| `redirect_uri` | string | 否* | 重定向URI（授权码流程时必需） |
...
| `redirect_uris` | array | 是 | 重定向URI列表 |
| `client_uri` | string | 否 | 客户端主页URI |
| `logo_uri` | string | 否 | 客户端Logo URI |
...
| `tos_uri` | string | 否 | 服务条款URI |
| `policy_uri` | string | 否 | 隐私政策URI |
